...
要約・詳細ダイジェストへの混入が過去の監査（2026-06-15 実施、対象 06-09〜06-15）で
確認された評価語・推測表現。`check.sh` が本文から検出し WARN を出す。
文脈次第で正当な用例（Critical の訳語、帰属付きの「推奨」等）があるため FAIL にはしない。

`注目` `話題` `画期的` `革新的` `高成長` `最高水準` `節目` `観測が強まる` `観測も強まる` `警告となる` `推奨される` `期待される`

**監査で新しい評価語の混入を検出したら、この行に追記すること。**
...
- **機械パース対象**: 「## NGドメイン」セクションの**バッククォート囲みドメイン**（例: `` `bloomberg.com` ``）を `cowork/scripts/check.sh` が直接読み込む（単一の正）。NGドメインは必ずこの形式で記載する。読み込みに失敗すると check.sh は exit 2 で停止する
- **機械パース対象（評価語）**: 「## NG評価語」セクションの**バッククォート囲み語**も check.sh が読み込み、本文検出時に WARN を出す。語は必ずバッククォートで囲むこと
