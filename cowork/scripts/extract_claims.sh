...
#
# 高リスク（フィルタ強度=厳しめ）= 金額(億/万/兆 ドル・円)、パーセント、
#            CVE-ID、CVSS スコア。資金調達・研究統計・脆弱性など、数値の
#            取り違え・捏造が起きやすく実害の大きい箇所に絞る。
#   ※ 軽微な件数(「12件の脆弱性修正」等)は実害が小さいため対象外。
...
# フィルタ強度=厳しめ（金額/%/CVE/CVSS）。範囲を広げる場合は末尾に
# |…(件|人|社|台|名|株|カ国|か国) を足す（現行=最も網羅、コスト高）。
RISK='(CVE-[0-9][0-9][0-9][0-9]-[0-9]|CVSS *[0-9]|[0-9]%|[0-9０-９][0-9０-９,，.]*(億|万|兆)?(ドル|円))'
