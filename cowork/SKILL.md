...
- **元記事リンク**：元記事のURL
- **公開日マーカー**：各リンク行の末尾に確認した公開日を `<!-- pub:YYYY-MM-DD -->` 形式で付与する（例：`→ [TechCrunch](https://...) <!-- pub:2026-06-09 -->`）。check.sh が7日以内かをオフライン検証するため必須。詳細は `TEMPLATE.md` 参照
...
### 選定・除外・品質ルール
選定・分類・除外・品質の詳細は **`SELECTION_RULES.md`（正本）** と `SOURCES.md`（ソース限定リスト）に従う。要点のみ再掲：
- 1週間以内・重複は1本・過去掲載URLは再採用しない（最終的に `check.sh` が機械検証）
- AI は原則「🤖 AI最新動向」、資金調達/IPO/M&A は「📰 ビジネス・経済」、「🚀 新サービス・ローンチ」は製品ローンチに限定
- ソース定義外ドメインは件数埋めでも不採用。反応補助ソースは反応欄/補足に限り、単独で事実を断定しない
- 最小件数を満たせなければ無理に埋めず「本日の更新なし」。見出しは具体的・事実ベース
